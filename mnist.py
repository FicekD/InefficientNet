...
def main():
    ...
    train_x, train_y = prepare_data(train_path, labels)
    val_x, val_y = prepare_data(val_path, labels)
    test_x, test_y = prepare_data(test_path, labels)

    classifier = ien.SequentialModel()
    classifier.add([
        ien.Input((img_size, img_size)),
        ien.Flatten(),
    ...
    classifier.summary()
    classifier.fit(train_x, train_y, 25, 64, val_x=val_x, val_y=val_y)
